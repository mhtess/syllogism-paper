...
\caption{Overview of the probabilistic pragmatics perspective of syllogistic reasoning. The reasoner first acts as a listener (left), interpreting the premises of the syllogism by updating its prior beliefs about the state (Venn diagram) according to the truth-functional meanings of the quantifier relations in the premises, resulting in a posterior distribution over states. Then, the reasoner acts as a speaker (right), deciding among a set of conclusions which would be the best to produce. The model developed in this paper restricts the listener model to only literal (though potentially noisy) interpretations of the premises, while the speaker model entertains pragmatic considerations (i.e., by reasoning about a listener).}
\label{fig:cartoon}
\end{figure}
...
The model's prior on states $P(s)$ is constructed by sampling a Bernoulli random variable for each of the $r \in R$ regions: $P(s) \propto \prod_{r \in R} P(r)$. %= (P(a) \cdot P(b) \cdot P(c)) ^n$$
...
\begin{figure}[t]
\centering
\includegraphics[width = 0.85\textwidth]{figs/bda_rsa_scatters_0paramPrior_nvc.pdf}
...
% We discuss the modeling approach in more detail, including how it can be extended to account for other phenomena in syllogistic reasoning, the implications of our findings in the context of other work on pragmatic reasoning with syllogisms.
...
%i think we can also say the basic point more succinctly (though then %walking through the example is good): a uniform noise rate in the likelihood and differential priors for different properties leads to differential rejection rates in the posterior, essentially due to bayes rule.
...
Though QUDs may not be the explicit question asked of a person \cite<e.g.,>{hawkins2015you, hawkins2017you}, in the context of a syllogistic reasoning problem, the most natural QUD is the explicit question asked of participants: What relationship between As and Cs follows from the premises? \cite{tessler2014some}.
